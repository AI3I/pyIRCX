...
from staff_commands import StaffCommandsMixin
...
staff_commands.CONFIG = CONFIG  # Share CONFIG with staff command handlers
...
from user import User, RateLimiter
...
class pyIRCXServer(StaffCommandsMixin):
    ...
    def __init__(self):
        ...
        self.ssl_manager = None  # Will be set by ServerManager.start()
    ...
    async def boot(self):
        ...
        if should_create_services:
            ...
            self._create_virtual_service('System', 'System', "System", omnipresent=True)
            ...
            self._create_virtual_service('ServiceBot', 'ServiceBot', "ServiceBot Pool Dispatcher")
...
# Continuing pyIRCXServer class...
    ...
    async def handle_webirc(self, user, params):
        ...
        # Verify password
        expected_password = str(gateway_config.get('password') or '')
        if not expected_password or not hmac.compare_digest(expected_password.encode(), password.encode()):
    ...
    async def handle_create(self, user, params):
        ...
        # Send MODE message showing applied modes ('c' is a CREATE flag, and
        # 'r'/'z' are rejected by _apply_create_modes)
        applied_modes = modes_str.translate(str.maketrans('', '', 'crz')) if modes_str else ''
        if len(applied_modes) > 1:
            mode_msg = f":{self.servername} MODE {chan_name} {applied_modes}"
            if mode_args:
    ...
    async def _apply_create_modes(self, channel, user, modes_str, mode_args):
        ...
        for mode_char in mode_chars:
            ...
            elif mode_char in ('r', 'z'):
                # Same rules as MODE: +r is never user-settable, and +z needs a
                # registered channel, which a freshly created one never is.
                message_key = 'mode_r_cannot_set' if mode_char == 'r' else 'mode_z_must_register'
                await user.send(self.get_reply("696", user, target=channel.name, mode=mode_char,
                                               message=SERVER_MESSAGES[message_key]))
            else:
...
class ServerManager:
    ...
    async def start(self):
        ...
        self.server.ssl_manager = self.ssl_manager  # STATS reads certificate info
...
# Get mode string with +r for registered channels
    ...
    async def shutdown(self):
        ...
        try:
            # Overall shutdown timeout of 10 seconds (wait_for rather than
            # asyncio.timeout(), which needs Python 3.11+)
            await asyncio.wait_for(self._shutdown_steps(), timeout=10)
        except asyncio.TimeoutError:
    ...
    async def _shutdown_steps(self):
        """Shutdown sequence run by shutdown() under its overall timeout."""
