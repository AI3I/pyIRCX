...
@runner.test("CREATE - Requires IRCX mode")
async def test_create_requires_ircx():
    """Test that CREATE requires IRCX mode"""
    client = IRCTestClient("create_ircx")

    await client.connect("CreateTest1")

    client.buffer.clear()
    await client.send_raw("CREATE #testcreate")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should get error about IRCX requirement
    has_ircx_error = any("IRCX" in line and "CREATE" in line for line in client.buffer)

    print(f"   IRCX mode error: {has_ircx_error}")

    assert has_ircx_error, "Should require IRCX mode"

    await client.disconnect()


@runner.test("CREATE - Simple channel creation")
async def test_create_simple():
    """Test creating a simple channel in IRCX mode"""
    client = IRCTestClient("create_simple")

    await client.connect("CreateTest2")
    ...
    client.buffer.clear()
    await client.send_raw("CREATE #simplecreate")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should get JOIN, NAMES, end of NAMES
    has_join = any("JOIN #simplecreate" in line for line in client.buffer)
    has_353 = any(" 353 " in line for line in client.buffer)
    has_366 = any(" 366 " in line for line in client.buffer)

    print(f"   JOIN message: {has_join}")
    print(f"   NAMES list: {has_353}")
    print(f"   End NAMES: {has_366}")
...
@runner.test("CREATE - With simple modes")
async def test_create_with_modes():
    """Test CREATE with modes (no arguments)"""
    client = IRCTestClient("create_modes")

    await client.connect("CreateTest3")
    await client.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client.buffer.clear()
    await client.send_raw("CREATE #modecreate mnt")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should get JOIN and MODE message showing applied modes
    has_join = any("JOIN #modecreate" in line for line in client.buffer)
    has_mode = any("MODE #modecreate" in line and "+mnt" in line for line in client.buffer)

    print(f"   JOIN message: {has_join}")
    print(f"   MODE message: {has_mode}")

    assert has_join, "Should receive JOIN confirmation"
    assert has_mode, "Should receive MODE message with applied modes"

    await client.disconnect()


@runner.test("CREATE - With mode arguments (limit)")
async def test_create_with_limit():
    """Test CREATE with user limit"""
    client = IRCTestClient("create_limit")

    await client.connect("CreateTest4")
    await client.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client.buffer.clear()
    await client.send_raw("CREATE #limitcreate ntl 50")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should get JOIN and MODE showing limit
    has_join = any("JOIN #limitcreate" in line for line in client.buffer)
    has_mode = any("MODE #limitcreate" in line and "50" in line for line in client.buffer)

    print(f"   JOIN message: {has_join}")
    print(f"   MODE with limit: {has_mode}")

    assert has_join, "Should receive JOIN confirmation"
    assert has_mode, "Should receive MODE message with limit"

    await client.disconnect()


@runner.test("CREATE - With channel key")
async def test_create_with_key():
    """Test CREATE with channel key"""
    client = IRCTestClient("create_key")

    await client.connect("CreateTest5")
    await client.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client.buffer.clear()
    await client.send_raw("CREATE #keycreate ntk secretkey")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should get JOIN and MODE showing key
    has_join = any("JOIN #keycreate" in line for line in client.buffer)
    has_mode = any("MODE #keycreate" in line and "secretkey" in line for line in client.buffer)

    print(f"   JOIN message: {has_join}")
    print(f"   MODE with key: {has_mode}")

    assert has_join, "Should receive JOIN confirmation"
    assert has_mode, "Should receive MODE message with key"

    await client.disconnect()


@runner.test("CREATE - Complex modes (key + limit)")
async def test_create_complex_modes():
    """Test CREATE with multiple mode arguments"""
    client = IRCTestClient("create_complex")

    await client.connect("CreateTest6")
    await client.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client.buffer.clear()
    await client.send_raw("CREATE #complexcreate mntkl 25 testkey")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should get JOIN and MODE with both key and limit
    has_join = any("JOIN #complexcreate" in line for line in client.buffer)
    has_mode = any("MODE #complexcreate" in line for line in client.buffer)
    has_limit = any("25" in line for line in client.buffer if "MODE" in line)
    has_key = any("testkey" in line for line in client.buffer if "MODE" in line)

    print(f"   JOIN message: {has_join}")
    print(f"   MODE message: {has_mode}")
    print(f"   Limit in MODE: {has_limit}")
    print(f"   Key in MODE: {has_key}")

    assert has_join, "Should receive JOIN confirmation"
    assert has_mode, "Should receive MODE message"

    await client.disconnect()


@runner.test("CREATE - +c flag with new channel (success)")
async def test_create_c_flag_success():
    """Test +c flag with non-existent channel"""
    client = IRCTestClient("create_c_success")

    await client.connect("CreateTest7")
    await client.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client.buffer.clear()
    await client.send_raw("CREATE #cflagsuccess c")
    await asyncio.sleep(0.3)
    await client.read_lines()

    # Should succeed and JOIN the channel
    has_join = any("JOIN #cflagsuccess" in line for line in client.buffer)

    print(f"   CREATE with +c succeeded: {has_join}")

    assert has_join, "Should create channel with +c flag"

    await client.disconnect()


@runner.test("CREATE - +c flag with existing channel (fail)")
async def test_create_c_flag_failure():
    """Test +c flag with existing channel should fail"""
    client1 = IRCTestClient("create_c_fail1")
    client2 = IRCTestClient("create_c_fail2")

    # First client creates channel
    await client1.connect("CreateTest8a")
    await client1.send_raw("IRCX")
    await asyncio.sleep(0.2)
    await client1.send_raw("CREATE #cflagfail")
    await asyncio.sleep(0.3)

    # Second client tries to CREATE with +c (should fail)
    await client2.connect("CreateTest8b")
    await client2.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client2.buffer.clear()
    await client2.send_raw("CREATE #cflagfail c")
    await asyncio.sleep(0.3)
    await client2.read_lines()

    # Should get ERR_CHANNELEXIST (926)
    has_error = any("926" in line or "already exists" in line.lower() for line in client2.buffer)

    print(f"   Channel exists error: {has_error}")

    assert has_error, "Should receive error that channel exists"

    await client1.disconnect()
    await client2.disconnect()


@runner.test("CREATE - Existing channel without +c (should join)")
async def test_create_existing_no_c_flag():
    """Test CREATE on existing channel without +c should join"""
    client1 = IRCTestClient("create_exist1")
    client2 = IRCTestClient("create_exist2")

    # First client creates channel
    await client1.connect("CreateTest9a")
    await client1.send_raw("IRCX")
    await asyncio.sleep(0.2)
    await client1.send_raw("CREATE #existtest")
    await asyncio.sleep(0.3)

    # Second client CREATEs same channel without +c (should just join)
    await client2.connect("CreateTest9b")
    await client2.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client2.buffer.clear()
    await client2.send_raw("CREATE #existtest")
    await asyncio.sleep(0.3)
    await client2.read_lines()

    # Should get JOIN (joined existing channel)
    has_join = any("JOIN #existtest" in line for line in client2.buffer)

    print(f"   Joined existing channel: {has_join}")

    assert has_join, "Should join existing channel"

    await client1.disconnect()
    await client2.disconnect()


@runner.test("CREATE - Mode normalization (with/without +)")
async def test_create_mode_normalization():
    """Test that modes work with or without + prefix"""
    client1 = IRCTestClient("create_norm1")
    client2 = IRCTestClient("create_norm2")

    # Test without + prefix
    await client1.connect("CreateTest10a")
    await client1.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client1.buffer.clear()
    await client1.send_raw("CREATE #normtest1 mnt")
    await asyncio.sleep(0.3)
    await client1.read_lines()

    has_mode1 = any("MODE #normtest1" in line and "+mnt" in line for line in client1.buffer)

    # Test with + prefix
    await client2.connect("CreateTest10b")
    await client2.send_raw("IRCX")
    await asyncio.sleep(0.2)

    client2.buffer.clear()
    await client2.send_raw("CREATE #normtest2 +mnt")
    await asyncio.sleep(0.3)
    await client2.read_lines()

    has_mode2 = any("MODE #normtest2" in line and "+mnt" in line for line in client2.buffer)

    print(f"   Modes without +: {has_mode1}")
    print(f"   Modes with +: {has_mode2}")

    assert has_mode1, "Modes should work without + prefix"
    assert has_mode2, "Modes should work with + prefix"
